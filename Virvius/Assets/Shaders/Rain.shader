Shader "Unlit/Rain"
{
    Properties
    {
        ...
        _DropSizeX("DropSizeX", float) = 3
        _DropSizeY("DropSizeY", float) = 1
        _TrailSizeX("TrailSizeX", float) = 2.1
        _TrailSizeY("TrailSizeY", float) = 0.5
    }
    SubShader
    {
        ...
        { 
            ...
            float4 _MainTex_ST;
            float _Size, _T, _Distortion, _Blur, _DropSizeX, _DropSizeY, _TrailSizeX, _TrailSizeY;
            ...
            float3 Layer(float2 UV, float t)
            {
                ...
                float dropX = _DropSizeX / 100;
                float dropY = _DropSizeY / 100;
                float trailX = _TrailSizeX / 100;
                float trailY = _TrailSizeY / 100;


                float2 dropPos = (gv - float2(x, y)) / aspect;
                float drop = S(dropX, dropY, length(dropPos));

                float2 trailPos = (gv - float2(x, t * 0.25)) / aspect;
                trailPos.y = (frac(trailPos.y * 8) -0.5) / 8;
                float trail = S(trailX, trailY, length(trailPos));
